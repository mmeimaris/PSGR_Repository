...
public class StatisticsHandler {
	...
	public StatisticsHandler(ArrayList<String[]> dates){
		...
		dbUrl = DailyRoutineHandler.connectionString;
		model = VirtModel.openDatabaseModel(null, DailyRoutineHandler.connectionString, "marios", "dirtymarios");
		...
		Model dayModel = VirtModel.openDatabaseModel(DailyRoutineHandler.dayGraphName, dbUrl, "marios", "dirtymarios");
		System.out.println("Fetching Types for Days.");
		typeDates(days, dayModel, "day", DailyRoutineHandler.dayGraphName);
		//topPaymentsForDecisionDates(dayModel, DailyRoutineHandler.dayGraphName, SPARQLQueries.daySelector, days, "days");
		dayModel.close();		
		totalAmountsForDecisionDates(DailyRoutineHandler.dayGraphName, SPARQLQueries.daySelector2);
		
		Model weekModel = VirtModel.openDatabaseModel(DailyRoutineHandler.weekGraphName, dbUrl, "marios", "dirtymarios");
		System.out.println("Fetching Types for Weeks.");
		typeDates(weeks, weekModel, "week", DailyRoutineHandler.weekGraphName);
		weekModel.close();
		totalAmountsForDecisionDates(DailyRoutineHandler.weekGraphName, SPARQLQueries.weekSelector2);
		
		Model monthModel = VirtModel.openDatabaseModel(DailyRoutineHandler.monthGraphName, dbUrl, "marios", "dirtymarios");
		System.out.println("Fetching Types for Months.");
		typeDates(months, monthModel, "month", DailyRoutineHandler.monthGraphName);
		monthModel.close();
		totalAmountsForDecisionDates(DailyRoutineHandler.monthGraphName, SPARQLQueries.monthSelector2);
		
		Model yearModel = VirtModel.openDatabaseModel(DailyRoutineHandler.yearGraphName, dbUrl, "marios", "dirtymarios");
		System.out.println("Fetching Types for Years.");
		typeDates(years, yearModel, "year", DailyRoutineHandler.yearGraphName);
		yearModel.close();
		totalAmountsForDecisionDates(DailyRoutineHandler.yearGraphName, SPARQLQueries.yearSelector2);
		//totalAmountsOverall();
		//binaryRelationships();		
	...
	public void handleCorrections(){
		...
		try{
			VirtuosoQueryExecution vqe2 = VirtuosoQueryExecutionFactory.create (SPARQLQueries.validityQuery(), graph);
			vqe2.execSelect();
			vqe2.close();
		}catch(Exception e){}
	}
