...
public class JsonWriter {
	...
	public String jsonBarWriter(ArrayList<String[]> writerList, String tag, String[] date, String title, String engTitle){
		...
		for(String[] writeArray : writerList){
			...
			category += "\"<a href=\\\"http://publicspending.medialab.ntua.gr/tab.php?uri=" +writeArray[5] + "\\\">"+ writeArray[2] + "</a>\",";
...
public String jsonBarWriterOriginal(ArrayList<String[]> writerList, String tag, String[] date, String title, String engTitle){
		...
		for(String[] writeArray : writerList){
			y += "{\"y\":" + writeArray[0] + "},";
			category += "\"" + writeArray[2] + "\",";
			//category += "\"<a href=\\\"" +writeArray[5] + "\\\">"+ writeArray[2] + "</a>\",";
			engCategory +="\"" + writeArray[4] + "\",";
			data += "{\"name\": \"" + writeArray[1] + "\", \"nameeng\": \"" + writeArray[3] + "\", \"y\": " + writeArray[0] + "}," ;
		}
		writable += category.substring(0,category.length()-1) + "],\n"+"\"categorieseng\": " + engCategory.substring(0,engCategory.length()-1) + "],\n" + "\"data\": " + data.substring(0, data.length()-1) + "]\n}]";
...
public String jsonBarWriter2(ArrayList<String[]> writerList, String tag, String[] date, String title, String engTitle){
...
public String jsonPayeesListWriter(ArrayList<String[]> list){
	...
	public String jsonTopPayersTableWriter(ArrayList<String[]> list){
		...
		while(it.hasNext()){
			...
			writable += "{\"payer\": \"<a href=\\\"tab.php?uri="+names[5]+"\\\">"+names[1]+"</a>\", \"payereng\" : \"<a href=\\\"tab.php?uri="+names[5]+"\\\">"+names[3]+"</a>\", \"order\":"+i+", \"amount\": \"€"+decorateAmount(names[0])+"\"},\n";
		}
	...
	public String jsonTopPayeesTableWriter(ArrayList<String[]> list){
		...
		while(it.hasNext()){
			i++;
			String[] names = (String[]) it.next();
			writable += "{\"payee\": \"<a href=\\\"tab.php?uri="+names[5]+"\\\">"+names[1]+"</a>\", \"payeeeng\" : \"<a href=\\\"tab.php?uri="+names[5]+"\\\">"+names[3]+"</a>\", \"order\":"+i+", \"amount\": \"€"+decorateAmount(names[0])+"\"},\n";
		}
		writable = writable.substring(0,writable.lastIndexOf(",")) + "]";
		return writable;
	}
